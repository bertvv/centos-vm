...
CENTOS7_X86_64 ?= http://buildlogs.centos.org/centos/7/os/x86_64-latest/images/boot.iso
...
SHORTCUT_TARGETS := centos7qa centos65 centos65-desktop centos64 centos64-desktop centos510 centos59 centos65-i386 centos64-i386 centos510-i386 centos59-i386
...
$(VMWARE_BOX_DIR)/centos7qa$(BOX_SUFFIX): centos7qa.json $(SOURCES) http/ks7.cfg
	rm -rf $(VMWARE_OUTPUT)
	mkdir -p $(VMWARE_BOX_DIR)
	packer build -only=$(VMWARE_BUILDER) $(PACKER_VARS) -var "iso_url=$(CENTOS7_X86_64)" $<
...
$(VIRTUALBOX_BOX_DIR)/centos7qa$(BOX_SUFFIX): centos7qa.json $(SOURCES) http/ks7.cfg
	rm -rf $(VIRTUALBOX_OUTPUT)
	mkdir -p $(VIRTUALBOX_BOX_DIR)
	packer build -only=$(VIRTUALBOX_BUILDER) $(PACKER_VARS) -var "iso_url=$(CENTOS7_X86_64)" $<
